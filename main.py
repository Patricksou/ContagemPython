...
def set_info(detec):
    ...
    for (x, y) in detec:
        if (pos_linha + offset) > y > (pos_linha - offset):
            animais += 1
            cv2.line(frame1, (50, pos_linha), (1300, pos_linha), (0, 127, 255), 3)
            detec.remove((x, y))
            print("Animais detectados até o momento: " + str(animais))


def show_info(frame1, dilatada):
    text = f'Animais: {animais}'
...
while True:
    ...
    cv2.line(frame1, (50, pos_linha), (1300, pos_linha), (255, 127, 0), 3)
